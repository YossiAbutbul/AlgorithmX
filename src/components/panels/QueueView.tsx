  ...
  return (
    <div>
      <div className="flex min-h-[42px] items-center gap-1.5" dir="rtl">
        <span className="text-[var(--step-1)] font-semibold text-ink-soft">ראש</span>
        <div className="flex flex-1 flex-wrap gap-1.5">
          {view.items.length === 0 && (
            <span className="text-[var(--step-1)] text-ink-soft">התור ריק</span>
          )}
          {view.items.map((it, i) => (
            <span
              key={`${it}-${i}`}
              ...
              className="num rounded-lg border px-2.5 py-1 text-[var(--step-2)] font-semibold"
              style={{
        ...
        <span className="text-[var(--step-1)] font-semibold text-ink-soft">זנב</span>
      </div>
