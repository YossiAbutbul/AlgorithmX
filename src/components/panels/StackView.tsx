  ...
  return (
    <div>
      <div className="flex min-h-[42px] flex-col-reverse gap-1">
        {items.length === 0 && (
          <span className="text-(length:--step-1) text-ink-soft">המחסנית ריקה</span>
        )}
        {items.map((it, i) => (
          <span
            key={`${it}-${i}`}
            ...
            className="item-in num rounded-lg border px-2.5 py-1 text-center text-(length:--step-2) font-semibold"
            style={{
          ...
          >
            {it}
            {i === items.length - 1 && (
              <span className="ms-2 text-(length:--step-1) font-normal text-ink-soft">ראש</span>
            )}
          </span>
        ))}
      </div>
